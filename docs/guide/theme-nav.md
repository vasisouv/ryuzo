...
By default, nav shows the title of the site referencing [`config.title`](components/app-configs#title) value. If you would like to change what's displayed on nav, you may define custom text in `themeConfig.siteTitle` option.
...
You can also pass an object as logo if you want to add `alt` attribute or customize it based on dark/light mode. Refer [`themeConfig.logo`](components/theme-configs#logo) for details.
...
Refer [`socialLinks`](components/theme-configs#sociallinks).
